...
2. **Check for a project-designated standard document** (e.g. `flutter-clean-architecture-guide.md` at repo root, or similarly named `*-architecture-guide.md`/`*-coding-standard.md` files). If one exists, read it directly and treat it as authoritative — condense its rules into `tech.md`/`structure.md` and reference it by name rather than re-deriving conventions from scratch.
3. Analyze codebase (JIT):
   - `glob_file_search` for source files
   - `read_file` for README, package.json/pubspec.yaml, etc.
   - `grep` for patterns
4. Extract patterns (not lists):
   - Product: Purpose, value, core capabilities
   - Tech: Frameworks, decisions, conventions
   - Structure: Organization, naming, imports
5. Generate steering files (follow templates)
6. Load principles from `.kiro/settings/rules/steering-principles.md`
