...
- **tech.md**: Key frameworks, standards, conventions, and the project's binding type-safety/naming rules (not all dependencies)
...
Some projects designate a single external document as the binding coding standard (e.g. a `*-clean-architecture-guide.md` at repo root). When one exists:
- Reference it explicitly from `tech.md`/`structure.md` rather than duplicating its full content — steering stays a short pointer plus a condensed summary of its most load-bearing rules.
- Treat every rule in that document as mandatory unless it explicitly says "guideline."
- Commands that design, generate tasks for, implement, or review code must read that document directly, not just its steering summary, since steering intentionally omits code-level examples and checklists.
