...
│       ├── pages/
│       └── widgets/
├── feature_b/              # (same pattern as feature_a)
└── core/
    └── domain_shared/       # Non-obvious: why this is shared, not per-feature
```
...
##### Service Interface
```dart
abstract class [ComponentName]Service {
  Future<Result<Failure, OutputType>> methodName(InputType input);
}
```
- All parameters and the return type must be explicit, concrete Dart types — no `dynamic`, no `Map<String, dynamic>` crossing this boundary.
- Preconditions:
- Postconditions:
- Invariants:
